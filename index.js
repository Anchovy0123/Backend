...
app.post("/login", async (req, res) => {
  try {
    ...
    // 2) หา user
    // If there are duplicate usernames, prefer the most recently added
    const [rows] = await db.query(
    ...
    if (!passOK) {
      // Fallback: if duplicates exist, try other rows for this username
      for (let i = 1; i < rows.length && !passOK; i++) {
        const candidate = rows[i];
        if (typeof candidate.password === "string" && candidate.password.startsWith("$2")) {
          passOK = await bcrypt.compare(password, candidate.password);
        } else {
          passOK = password === String(candidate.password);
          if (passOK) {
            const newHash = await bcrypt.hash(password, 10);
            await db.query("UPDATE tbl_users SET password = ?, updated_at = NOW() WHERE id = ?", [newHash, candidate.id]);
